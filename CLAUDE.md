...
### コンフリクトを避ける：1 つずつ終わらせる

同じファイルを別々のブランチで同時に変えると、マージのときに**コンフリクト**（Git がどちらの変更を残すか決められない状態）が起きる。
このプロジェクトは 1 人で順番に進めるので、**未マージの PR を残したまま次の作業を始めない**ことで防ぐ。

- 新しい作業を始める前に、未マージの PR が無いか確認する（`gh pr list --author @me`）
- 画面確認待ちなどの PR があれば、**先にその確認・マージを済ませる**（確認待ちの間はコードを書かず、相談や調査にとどめる）
- 次のブランチは、上の手順どおり**必ず最新の main から**作る（`git pull origin main` の後）
