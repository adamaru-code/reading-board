...
[要件定義書](requirements.md) を土台に、実装に向けた **How** を図で整理する。
図は GitHub 上で描画される [Mermaid](https://mermaid.js.org/) 記法で記述している。

対応関係：要件定義書の §5 ユースフロー・§6 画面仕様・§7 データモデル・§8 API を、
本書で「遷移／構造／流れ」の観点から図示する。
...
画面は要件定義書 §6 の S1〜S4。ボードを中心に、追加/編集はモーダルで開いて閉じると戻る。
...
要件定義書 §4.7 の通り、将来的に単一ユーザー認証を追加する場合は `users` を追加し、
...
- API クライアントは `/api` を基点に CRUD をまとめる（実装ステップ §10-4 で作成）。

> 詳細なバリデーション仕様・コンポーネント分割・状態管理ライブラリの要否は、
> 各実装ステップ（[要件定義書](requirements.md) §10）で詰める。
