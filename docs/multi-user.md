...
> **ステータス：PR1〜5 実装済み（パスワードリセットは当面、管理者発行のリンク。メール送信は SES 導入時）。** 登録方式は**招待制**（管理者が画面から発行・1 回限り・有効期限 7 日）に決定（2026-09-23）。
...
| パスワードリセット（管理者が再設定リンクを発行） | 済（#137） |
| パスワードリセット（メールで自動送信） | 未（SES 導入時） |
| アカウント削除 | 済（#135） |
...
| 4 | アカウント削除（済） | 小 | 2 |
| 5 | パスワードリセット（管理者発行のリンク）（API 済） | 中 | 2 |
| 5b | パスワードリセットのメール自動送信 | 小 | AWS（SES）または別のメール送信サービス |
